...
import { resolveStapleInstanceId } from "../home-paths.js";
...
export function createBetterAuthInstance(db: Db, config: Config, trustedOrigins?: string[]) {
  const baseUrl = config.authBaseUrlMode === "explicit" ? config.authPublicBaseUrl : undefined;
  const secret = process.env.BETTER_AUTH_SECRET ?? process.env.STAPLE_AGENT_JWT_SECRET;
  if (!secret) {
    throw new Error(
      "BETTER_AUTH_SECRET (or STAPLE_AGENT_JWT_SECRET) must be set. " +
      "For local development, set BETTER_AUTH_SECRET=staple-dev-secret in your .env file.",
    );
  }
  const effectiveTrustedOrigins = trustedOrigins ?? deriveAuthTrustedOrigins(config);

  const publicUrl = process.env.STAPLE_PUBLIC_URL ?? baseUrl;
