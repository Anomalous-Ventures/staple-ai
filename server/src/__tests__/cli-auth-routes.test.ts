...
describe.sequential("cli auth routes", () => {
  ...
  it.sequential("creates a CLI auth challenge with approval metadata", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        command: "stapleai company import",
        clientName: "stapleai cli",
  ...
  it.sequential("rejects anonymous access to generic skill documents", async () => {
    ...
    const skillRes = await request(skillApp).get("/api/skills/staple");
  ...
  it.sequential("serves the invite-scoped staple skill anonymously for active invites", async () => {
    const invite = {
      id: "invite-1",
      companyId: "company-1",
    ...
    const res = await request(app).get("/api/invites/token-123/skills/staple");

    expect(res.status).toBe(200);
    expect(res.headers["content-type"]).toContain("text/markdown");
    expect(res.text).toContain("# Staple Skill");
  });
